...
export async function handleUploadAndPollStatus(page: Page) {
  ...
  if (!taskId)
    throw new Error('❌ No task_id found in final upload-chunk response.');
  ...
  const statusResponse = await page.waitForResponse(
    ...
    { timeout: 300000 } // Total timeout based on max attempts
  );
  ...
  const finalResponse = await page.waitForResponse(
    ...
    { timeout: 300000 } // Remaining timeout
  );
