...
test.describe('testUser1', () => {
  test('Process button disabled when file uploaded but no censor words', async ({
    ...
    await helpers.audioProcessingPage.uploadAudioFile(TestData.files.audio);
  ...
  test('Process button disabled when censor words entered but no file', async ({
    page,
  }) => {
    const helpers = new TestHelpers(page);
  ...
  test('Process button enabled when both file and censor words provided', async ({
    page,
  }) => {
    const helpers = new TestHelpers(page);
