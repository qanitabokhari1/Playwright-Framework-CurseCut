...
test.describe('Processing tags verification - "browser" vs "premium" (data attributes)', () => {
  test('browser tag verification (data attribute)', async ({ page }) => {
    ...
    await expect(audioPage.pageInstance.getByText('Premium', { exact: true })).toBeVisible();
    ...
    await expect(audioPage.pageInstance.getByText('Browser', { exact: true })).toBeVisible();
